...
interface Table { id: number; tableNumber: number; status?: string; }
interface Category { id: number; name: string; }
interface Menu { id: number; name: string; price: number; image: string; categoryId: number; category?: Category; }
...
export default function AdminDashboard() {
  ...
  const handleAddTable = async (e: React.FormEvent) => {
    ...
    try {
      await axios.post(`${API_URL}/table`, { tableNumber: Number(newTableNumber), status: "EMPTY" }, { headers: { Authorization: `Bearer ${token}` } });
      setNewTableNumber(""); fetchTables(); 
    } catch (error) { alert("Gagal menambah meja."); } finally { setIsSubmittingTable(false); }
  };

  const handleDeleteTable = async (id: number) => {
    if (!window.confirm("Yakin ingin menghapus meja ini?")) return;
    try {
      await axios.delete(`${API_URL}/table/${id}`, { headers: { Authorization: `Bearer ${token}` } });
      fetchTables(); 
  ...
  const handleAddCategory = async (e: React.FormEvent) => {
    ...
    try {
      await axios.post(`${API_URL}/category`, { name: newCategoryName }, { headers: { Authorization: `Bearer ${token}` } });
      setNewCategoryName(""); fetchCategories(); 
    } catch (error) { alert("Gagal menambah kategori."); } finally { setIsSubmittingCategory(false); }
  };

  const handleDeleteCategory = async (id: number) => {
    if (!window.confirm("Yakin ingin menghapus kategori ini?")) return;
    try {
      await axios.delete(`${API_URL}/category/${id}`, { headers: { Authorization: `Bearer ${token}` } });
      fetchCategories(); 
  ...
  const handleDeleteMenu = async (id: number) => {
    ...
    try {
      await axios.delete(`${API_URL}/menu/${id}`, { headers: { Authorization: `Bearer ${token}` } });
      fetchMenus(); 
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#F4F5F7] space-y-4">
        ...
        <span className="font-bold text-slate-500 text-sm tracking-widest uppercase">Memuat Admin Panel...</span>
      </div>
    );
  }
        ...
        {activeTab === "CATEGORY" && (
          <div className="animate-[slideUp_0.3s_ease-out]">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              <div className="lg:col-span-1">
                <div className="bg-white p-8 rounded-[1.5rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100">
                  <h3 className="font-extrabold text-lg mb-6 text-slate-900">Tambah Kategori</h3>
              ...
              <div className="lg:col-span-2">
                <div className="bg-white p-8 rounded-[1.5rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100">
                  ...
                  {categories.length === 0 ? ( 
                    <p className="text-slate-400 text-center py-10 font-medium">Belum ada kategori terdaftar.</p> 
                  ) : (
                ...
                <div className="bg-white p-8 rounded-[1.5rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100">
                  ...
                  {menus.length === 0 ? ( 
                    <div className="flex flex-col items-center justify-center py-20 bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200">
                      ...
                      <p className="text-slate-400 font-medium text-sm">Belum ada menu yang didaftarkan.</p>
                    </div>
                  ) : (
