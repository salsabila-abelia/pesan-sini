      ...
      <div className="bg-white max-w-md w-full rounded-[2.5rem] shadow-[0_8px_30px_rgb(0,0,0,0.06)] border border-slate-100 p-8 sm:p-10 relative overflow-hidden">
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 mb-6 flex items-start gap-3 shadow-sm">
          ...
          <div>
            ...
            <p className="text-[13px] font-medium text-amber-700/90 mt-0.5 leading-relaxed">
              Pastikan pesanan Anda sudah sesuai. Pesanan yang telah dibayar <strong>tidak dapat dibatalkan</strong> atau diubah.
            </p>
          </div>
        </div>
        ...
        {paymentMethod === "CASH" ? (
          <div className="text-center bg-slate-50 border border-slate-100 rounded-[1.5rem] p-6 mt-8">
            ...
            <p className="text-slate-500 text-sm mb-8 leading-relaxed font-medium">
              Silakan menuju meja kasir dan sebutkan nomor pesanan Anda{" "}
              <strong className="text-slate-900">(#{orderId})</strong> untuk
              melakukan pembayaran agar pesanan segera diproses.
            </p>
          ...
          <div className="mt-4">
            {!uploadSuccess && (
              <p className="text-slate-500 text-sm text-center mb-6 font-medium">
                Pindai kode QRIS di bawah ini untuk menyelesaikan pembayaran,
                kemudian unggah struk buktinya.
              </p>
            )}

            {uploadSuccess ? (
              <div className="text-center p-6 bg-emerald-50 border border-emerald-100 rounded-[1.5rem] mt-6">
                <div className="w-14 h-14 bg-emerald-500 text-white rounded-full flex items-center justify-center mx-auto mb-4 text-2xl font-bold shadow-md shadow-emerald-500/30">
                  ✓
                </div>
                <h2 className="text-lg font-extrabold text-emerald-800 mb-2">
                  Bukti Terkirim!
                </h2>
                <p className="text-emerald-700/80 text-sm mb-8 font-medium">
                  Kasir sedang memverifikasi pembayaran Anda. Silakan tunggu di
                  meja.
                </p>
                <button
                  onClick={() => router.push("/")}
              ...
              <form onSubmit={handleUploadSubmit} className="space-y-6">
                <div className="bg-slate-50 border border-slate-100 rounded-[1.5rem] p-5 flex flex-col items-center shadow-inner">
                  ...
                  <div className="w-48 h-48 bg-white p-3 rounded-2xl border border-slate-200 flex items-center justify-center shadow-sm relative group">
                    <img
                      src="/images/qris-code.jpeg"
                      alt="QRIS Merchant Barcode"
                      className="w-full h-full object-contain"
                      onError={(e) => {
                        (e.target as HTMLImageElement).src =
                          "https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=PesanSiniCafe";
                      }}
                    />
                  </div>

                  <h3 className="font-black text-slate-800 text-base mt-4 tracking-tight">
                    PESAN-SINI CAFE
                  </h3>
                  <p className="text-[10px] text-slate-400 font-bold tracking-wider uppercase mt-0.5">
                    NMID : ID1023256789123
                  </p>
                </div>
                  ...
                  {previewUrl ? (
                    ...
                    <div className="py-4">
                      ...
                      <p className="text-[11px] text-slate-400 mt-1 font-medium">
                        Tap untuk memilih screenshot / foto struk
                      </p>
                    </div>
                  )}
                  <input
